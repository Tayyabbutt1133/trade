    ...
    <section className="py-16 px-4 bg-[#FBFBFB]">
      ...
      <div className="space-y-8">
        <div className="space-y-4">
          <h2 className={`text-gray-600 ${fonts.roboto}`}>Featured Suppliers</h2>
            <h3 className={`text-4xl md:max-w-[700px] font-semibold text-gray-900 ${fonts.montserrat}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURED_SUPPLIERS.map((supplier) => (
              // <SupplierCard key={supplier.id} supplier={supplier} />
               <div key={supplier.id} className="relative aspect-square overflow-hidden rounded-lg group">
                    {/* Background Image */}
                    <Image
